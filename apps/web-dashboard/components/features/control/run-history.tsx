 ...
 * Implementa la ventana de visualización hacia el archivo inmutable
 * del Motor B. Consume reportes certificados directamente desde
 * el Cuartel General para análisis de rastro forense.
 *
 * # Higiene Técnica:
 * - Eliminación de 'React' import innecesario para satisfacer TS6133.
 * - Sincronización con el contrato ArchivedJob (L2).
 * =================================================================
 */
 ...
 *
 * @returns {JSX.Element} El componente de rastro de auditoría.
 */
export function RunHistory() {
  /**
   * ADQUISICIÓN DE DATOS ESTRATÉGICOS (L4)
   * Recupera los últimos 10 trabajos archivados para análisis de tendencia.
   */
  const { data: archives, isLoading } = useQuery<ArchivedJob[]>({
    queryKey: ["run-history-archive-v26"],
    queryFn: () => strategicArchive.getHistory(10),
    staleTime: 60000, // Datos inmutables: 1 minuto de persistencia en caché.
  });
  ...
  return (
    <Card className="bg-[#050505] border-slate-800 flex flex-col h-full shadow-2xl relative overflow-hidden group">
      ...
      {/* FOOTER TÉCNICO DE CONTEXTO (MOTOR B STATUS) */}
